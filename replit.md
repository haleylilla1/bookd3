...
- **Technical Implementations**:
    ...
    - **"Got Paid" Workflow**: Complete additive workflow for tracking income and expenses with fields for `total_received`, `reimbursed_parking`, `reimbursed_other`, `unreimbursed_parking`, `unreimbursed_other`, and `mileage`. Automatically calculates actual pay and business deductions including IRS standard mileage rate ($0.655/mile for 2024). Integrates with dashboard and report updates. Fixed critical mileage saving bug (August 2025) - mileage now properly saves to database and appears in reports.
    - **Emergency BA Email Notifications**: Automatic email notifications to agencies when Brand Ambassadors apply to emergency gigs using SendGrid integration. Includes gig details and BA contact information for direct follow-up.
